...
`)),this._prevFrame=u;return}this.output.write(srcExports.erase.down());}this.output.write(u),this.state==="initial"&&(this.state="active"),this._prevFrame=u;}}}var SD=Object.defineProperty,$D=(t,u,F)=>u in t?SD(t,u,{enumerable:true,configurable:true,writable:true,value:F}):t[u]=F,q=(t,u,F)=>($D(t,typeof u!="symbol"?u+"":u,F),F);class jD extends x{constructor(u){super(u,false),q(this,"options"),q(this,"cursor",0),this.options=u.options,this.cursor=this.options.findIndex(({value:F})=>F===u.initialValue),this.cursor===-1&&(this.cursor=0),this.changeValue(),this.on("cursor",F=>{switch(F){case "left":case "up":this.cursor=this.cursor===0?this.options.length-1:this.cursor-1;break;case "down":case "right":this.cursor=this.cursor===this.options.length-1?0:this.cursor+1;break}this.changeValue();});}get _value(){return this.options[this.cursor]}changeValue(){this.value=this._value.value;}}

function ce(){return g__default.platform!=="win32"?g__default.env.TERM!=="linux":!!g__default.env.CI||!!g__default.env.WT_SESSION||!!g__default.env.TERMINUS_SUBLIME||g__default.env.ConEmuTask==="{cmd::Cmder}"||g__default.env.TERM_PROGRAM==="Terminus-Sublime"||g__default.env.TERM_PROGRAM==="vscode"||g__default.env.TERM==="xterm-256color"||g__default.env.TERM==="alacritty"||g__default.env.TERMINAL_EMULATOR==="JetBrains-JediTerm"}const V=ce(),u=(t,n)=>V?t:n,le=u("\u25C6","*"),L=u("\u25A0","x"),W=u("\u25B2","x"),C=u("\u25C7","o"),ue=u("\u250C","T"),o=u("\u2502","|"),d=u("\u2514","\u2014"),k=u("\u25CF",">"),P=u("\u25CB"," "),w=t=>{switch(t){case "initial":case "active":return pc.cyan(le);case "cancel":return pc.red(L);case "error":return pc.yellow(W);case "submit":return pc.green(C)}},B=t=>{const{cursor:n,options:s,style:r}=t,i=t.maxItems??Number.POSITIVE_INFINITY,a=Math.max(process.stdout.rows-4,0),c=Math.min(a,Math.max(i,5));let l=0;n>=l+c-3?l=Math.max(Math.min(n-c+3,s.length-c),0):n<l+2&&(l=Math.max(n-2,0));const $=c<s.length&&l>0,p=c<s.length&&l+c<s.length;return s.slice(l,l+c).map((M,v,x)=>{const j=v===0&&$,E=v===x.length-1&&p;return j||E?pc.dim("..."):r(M,v+l===n)})},ve=t=>{const n=(s,r)=>{const i=s.label??String(s.value);switch(r){case "selected":return `${pc.dim(i)}`;case "active":return `${pc.green(k)} ${i} ${s.hint?pc.dim(`(${s.hint})`):""}`;case "cancelled":return `${pc.strikethrough(pc.dim(i))}`;default:return `${pc.dim(P)} ${pc.dim(i)}`}};return new jD({options:t.options,initialValue:t.initialValue,render(){const s=`${pc.gray(o)}
${w(this.state)}  ${t.message}
`;switch(this.state){case "submit":return `${s}${pc.gray(o)}  ${n(this.options[this.cursor],"selected")}`;case "cancel":return `${s}${pc.gray(o)}  ${n(this.options[this.cursor],"cancelled")}
${pc.gray(o)}`;default:return `${s}${pc.cyan(o)}  ${B({cursor:this.cursor,options:this.options,maxItems:t.maxItems,style:(r,i)=>n(r,i?"active":"inactive")}).join(`
${pc.cyan(o)}  `)}
${pc.cyan(d)}
`}}}).prompt()},xe=(t="")=>{process.stdout.write(`${pc.gray(d)}  ${pc.red(t)}

`);},Ie=(t="")=>{process.stdout.write(`${pc.gray(ue)}  ${t}
`);},Se=(t="")=>{process.stdout.write(`${pc.gray(o)}
${pc.gray(d)}  ${t}
...
const sourceDir = path__default.dirname(__dirname);
const targetDir = process.cwd();
async function handleCopyFile(fileName) {
  try {
    const sourcePath = path__default.join(sourceDir, "templates", fileName);
    const targetPath = path__default.join(targetDir, fileName);
    if (fs__default.existsSync(targetPath)) {
      throw new Error(`The folder ${fileName} already exists. \u{1F916}`);
    }
    fs__default.cpSync(sourcePath, targetPath, { recursive: true });
    return pc.green(`Successfully copied ${fileName}\uFF01\u{1F389}`);
  } catch (error) {
...
async function handlePrompts() {
  Ie(`${packageJson.name}@${packageJson.version}`);
  const { monorepo, framework } = await Ce(
    {
      monorepo: () => ve({
    ...
    {
      onCancel: () => {
        xe(pc.white("Until next time! \u{1F44B}"));
        process.exit(0);
      }
    }
  );
